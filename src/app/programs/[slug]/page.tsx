...
export default function ProgramPage({ params }: { params: { slug: string } }) {
  ...
  const programBonuses = bonuses.filter(b => b.program_id === slug && b.status === "live");
  const pastBonuses = bonuses.filter(b => b.program_id === slug && b.status !== "live");
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-[#0F172A]">
      ...
      <main className="mx-auto max-w-6xl px-6 py-12">
        <div className="mb-8 flex items-center gap-6">
            <img src={program.logo_url} alt={program.name} className="h-24 w-24 rounded-full bg-white border border-[#E2E8F0] p-2 object-contain shadow-sm" />
            <div>
                <h1 className="text-4xl font-bold text-[#0F172A]">{program.name}</h1>
                <p className="mt-2 text-lg text-[#64748B]">{program.description}</p>
                <div className="mt-4 flex gap-4">
                    <span className="rounded-full bg-[#EFF6FF] px-3 py-1 text-sm font-semibold text-[#1D4ED8]">{program.valuation_cpp}¢ per point</span>
                    <span className="rounded-full bg-[#F1F5F9] px-3 py-1 text-sm font-semibold text-[#475569]">{programBonuses.length} Active Bonuses</span>
                </div>
            </div>
        </div>

        <section className="mb-12">
            <h2 className="text-2xl font-bold text-[#0F172A] mb-6">Active Bonuses</h2>
            {programBonuses.length > 0 ? (
                <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    {programBonuses.map(bonus => (
                        <div key={bonus.id} className="rounded-2xl border border-[#E2E8F0] bg-white p-6 shadow-sm">
                            ...
                            <div className="text-sm font-medium text-[#64748B]">Transfer to</div>
                            <div className="text-lg font-bold text-[#0F172A]">{partners.find(p => p.id === bonus.partner_id)?.name}</div>
                            <div className="mt-4 text-xs text-[#94A3B8]">Ends {new Date(bonus.end_date).toLocaleDateString()}</div>
                        </div>
                    ))}
                </div>
            ) : (
                <p className="text-[#64748B]">No active bonuses right now.</p>
            )}
        </section>

        <section>
            <h2 className="text-2xl font-bold text-[#0F172A] mb-6">Past Bonuses</h2>
            <div className="overflow-hidden rounded-xl border border-[#E2E8F0] bg-white shadow-sm">
                <table className="min-w-full divide-y divide-[#E2E8F0]">
                    <thead className="bg-[#F8FAFC]">
                        <tr>
                            <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-[#64748B]">Partner</th>
                    ...
                    <tbody className="divide-y divide-[#E2E8F0] bg-white">
                        {pastBonuses.map(bonus => (
                            <tr key={bonus.id} className="hover:bg-[#F8FAFC]">
                                <td className="px-6 py-4 text-sm font-medium text-[#0F172A]">{partners.find(p => p.id === bonus.partner_id)?.name}</td>
